...
for(const file of commandFiles){
  ...
  console.log("");                                  
  console.log((chalk.yellow(`                                                               Crée par Stitch#5600 !`)));  
  ...
  client.setActivity(`*help - *invite`);
});
...
client.on('message',  msg => {
  ...
  /*                   CMD LIAGE                                           */
    ...
    if(msg.content.startsWith('*invite')){
      const linkinv = new Discord.MessageEmbed()
      .setColor('#0099ff')
      .setDescription(":white_check_mark: Voici votre invitation : \n **https://discord.com/oauth2/authorize?client_id=842505013737422929&permissions=8&scope=bot**")
      msg.channel.send(linkinv);
    }
...
//Commande fake Image
    ...
    const embed = new Discord.MessageEmbed()
      .setDescription(`**${member.user.username}** te fait un bisous`, msg.author.avatarURL)
      .setImage(kiss[Math.floor(Math.random() * kiss.length)])
      .setFooter('Commande faite par ' + member.user.username)
      .setColor("RANDOM")
...
client.login("ODQyNTA1MDEzNzM3NDIyOTI5.YJ2R8A.uD8F1in9S0mdHdIHd3CZ0gxYKy4");
